...
@section('content')
            ...
            <div class="row no-gutters position-relative">
                    ...
                    <div class="category-sidebar">
                        <div class="all-category d-none d-lg-block">
                            ...
                            <a href="{{ route('categories.all') }}">
                                <span class="d-none d-lg-inline-block badge badge-warning" style="font-size: 18px; font-weight: bolder;">{{__('See More')}} ></span>
                            </a>
                        </div>
                ...
                <div class="@if($num_todays_deal > 0) col-lg-7 @else col-lg-9 @endif order-1 order-lg-0">
                    ...
                    {{-- <div class="trending-category  d-none d-lg-block">
                        <ul>
                            @foreach (\App\Models\Category::where('featured', 1)->get()->take(7) as $key => $category)
                ...
                <div class="col-lg-2 d-none d-lg-block">
                    <div class="flash-deal-box bg-white h-100">
                        ...
                        <div class="flash-content c-scrollbar c-height">
                            @foreach (filter_products(\App\Models\Product::where('published', 1)->where('todays_deal', '1'))->orderBy('updated_at','desc')->get() as $key => $product)
                                @if ($product != null)
            ...
            <div class="px-2 py-4 p-md-4 bg-white shadow-sm">
                <div class="section-title-1 clearfix ">
                    ...
                    <ul class="inline-links float-right">
                        <li><a href="{{ route('flash-deal-details', $flash_deal->slug) }}" class="active">{{ __('View More')}}</a></li>
                    </ul>
                </div>
    ...
    <div class="mb-4">
        <div class="container">
            <div class="row gutters-10">
                @foreach (\App\Models\Banner::where('position', 1)->where('published', 1)->get() as $key => $banner)
                    <div class="col-lg-{{ 12/count(\App\Models\Banner::where('position', 1)->where('published', 1)->get()) }}">
   ...
   @endif
    ...
    <div class="mb-4">
        <div class="container">
            <div class="row gutters-10">
                @foreach (\App\Models\Banner::where('position', 2)->where('published', 1)->get() as $key => $banner)
        ...
        <div class="container">
            <div class="row gutters-10">
                ...
                {{-- <div class="col-lg-6">
                    <div class="section-title-1 clearfix">
                        <h3 class="heading-5 strong-700 mb-0 float-left">
                            <span class="mr-4">{{__('Top 10 Sellers')}} <span>({{__('The Most Orders')}})</span></span>
                        </h3>
                    </div>
                    @php
                            $sellers = \App\Models\User::with('orders')->where('user_type','seller')->get();
                            foreach($sellers as $raw){
                                $raw->order_count =$raw->order_out_website + $raw->order_in_website;
                                $raw->save();
                            }
                    @endphp
                    <div class="row gutters-5">
                        @foreach (\App\Models\User::where('user_type','seller')->orderBy('order_count','desc')->get()->take(10) as $user)
                            @if($user->order_count != 0)
                            <div class="mb-3 col-6">
                                <a href="" class="bg-white border d-block c-base-2 box-2 icon-anim pl-2">
                                    <div class="row align-items-center no-gutters">
                                        <div class="col-3 text-center">
                                            <img src="{{ asset('frontend/images/placeholder.jpg') }}" data-src="{{ asset($user->avatar_original) }}"  class="img-fluid img lazyload">
                                        </div>
                                        <div class="info col-7">
                                        <div class="name text-truncate pl-3 py-4">{{ $user->name }}</div>
                                        </div>
                                    </div>
                                </a>
                            </div>
                ...
                {{-- <div class="col-lg-6">
                    <div class="section-title-1 clearfix">
                        <h3 class="heading-5 strong-700 mb-0 float-left">
                            <span class="mr-4">{{__('Top 10 Sellers')}} <span>({{__('The Most Profits')}})</span></span>
                        </h3>
                    </div>
                    <div class="row gutters-5">
                        @foreach (\App\Models\User::where('user_type','seller')->orderBy('balance_count','desc')->get()->take(10) as $user)
                            <div class="mb-3 col-6">
                                <a href="" class="bg-white border d-block c-base-2 box-2 icon-anim pl-2">
                                    <div class="row align-items-center no-gutters">
                                        <div class="col-3 text-center">
